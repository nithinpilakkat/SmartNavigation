...
unsigned char GsmCheckMsg()
	...
	char  *ResponseMsg , *ResEOF,*EOF_BidId,*EOF_BidAddress,*SOF;	
	ResponseMsg 	= 	NULL;
 	ResEOF 			= 	NULL;	
	EOF_BidId		=	NULL;
	EOF_BidAddress  =	NULL;
		...
		case MsgStarted:
			...
			case T_CsqReceived:
				...
				case EvtGsm:
							if(Gsm.BuffCnt>5)
							{
								ResponseMsg=	strstr(&Gsm.Rxbuff[1],"SM");	
							if(ResponseMsg!=NULL)
								{
									Gsm.Event=EvtGsmMsg;
									Gsm.Sms_Index			=	1;
							...
							if(Gsm.BuffCnt>5)
							{
								ResponseMsg=	strstr(&Gsm.Rxbuff[1],"RING");	
							if(ResponseMsg!=NULL)
								{
				...
				case EvtSmsRead:
							if(Gsm.BuffCnt>5)
							{
								ResponseMsg=	strstr(&Gsm.Rxbuff[1],"CMGR");	
							if(ResponseMsg!=NULL)
								{
									ResEOF=	strstr(&Gsm.Rxbuff[1],",,");
								      if(ResEOF!=NULL)
										{
										Cnt=0;	
										*ResEOF=0;	
										ResEOF=NULL;
										ResponseMsg=	strstr(&Gsm.Rxbuff[1],":SL1708300");
										if(ResponseMsg!=NULL)
										{
/*******Device ID*******/											
										SOF		=	strstr(ResponseMsg+1,",");
										ResEOF	=	strstr(SOF+1,",");
											if(ResEOF!=NULL)
											{
											*ResEOF=0;
											strcpy(Gsm.D_Id,SOF+1);
											SOF=ResEOF+1;
											Cnt++;
											}
/*******SERVER_IP******/	
...
/*******Counter******/	
				...
				default:
						...
						if(Gsm.BuffCnt>5)
							{
								ResponseMsg=	strstr(&Gsm.Rxbuff[1],";;");	
							if(ResponseMsg!=NULL)
								{
								    ResEOF=	strstr(&Gsm.Rxbuff[1],"#");
								      if(ResEOF!=NULL)
										...
										*ResEOF=0;	
										...
										EOF_BidAddress  =	strstr(&Gsm.Rxbuff[1],"~");
										if(EOF_BidAddress!=NULL)
										{
											EOF_BidId		=	strstr(&Gsm.Rxbuff[1],",");
											if(EOF_BidId!=NULL)
											{
												strcpy(Gsm.Bidding_Amt,EOF_BidId+1);
												*EOF_BidId	= 0;
												
											}
											else
											{
												strcpy(Gsm.Bidding_Id,EOF_BidAddress+1);
												Navigation.BIDDING_ID=atoi(&Gsm.Bidding_Id[0]);
												*EOF_BidAddress=0;
												Navigation.Bidding_State=Bidding_Init;
											}
										}
										strcpy(Gsm.ServerMsg,ResponseMsg+2);
										#ifdef DebugOn
									    GsmDisplayUpdate(Gsm.ServerMsg,2)	;
										#endif	
										...
										*ResEOF=0;	
										...
										if(EOF_BidAddress!=NULL)
										{
											...
											else
											{
												...
												Navigation.BIDDING_ID=atoi(&Gsm.Bidding_Id[0]);
												*EOF_BidAddress=0;
												Navigation.Bidding_State=Bidding_msg_Received;
											}
										}
										strcpy(Gsm.ServerMsg,ResponseMsg+2);
										#ifdef DebugOn

...
void Tcp()
{
	...
	switch (GsmPtr->Event)
	{
		...
		switch (GsmPtr->State)
				...
				case SMS_RECEIVED:
					#ifdef DebugOn
						   GsmDisplayUpdate("Sms Received   ",1)	;
					#endif
					GsmPtr->Sms_Index=0;
					drv_sim900_send_cmd(8,"AT+CMGR=1");          
     				//drv_sim900_send_cmd(get_size(str_buf1),str_buf1);
     				drv_sim900_send_cmd(2,"\r\n");
					GsmConfig(Con_Running,T_CCIPSTATUS, SMS_RECEIVED,MsgSend,	EvtSmsRead,Sec(1),GsmPtr->MaxDelay,TimerStart);					
				...
				break;//T_CLPORT
					...
					if(TCP_Delay.Function==FnEnter)
					{
						...
						GsmConfig(T_CCIPSTATUS,T_CCIPSTATUS, T_CCIPSTATUS,MsgSend,	TCP_SEND_OK,Sec(3),Sec(20),TimerStart);								
					}
					else
					{
						GsmPtr->State=T_CCIPSTATUS;	
